...
The site fetches from the following feeds by default:
...
Each feed displays up to 9 most recent items.

## How It Works

The `feed-data.json` file is automatically generated by a GitHub Actions workflow that runs the `scripts/fetch_feed.py` Python script. This script:
- Fetches multiple RSS/Atom feeds specified in the `FEED_URLS` environment variable
- Parses each feed using feedparser, requests, and BeautifulSoup
- Extracts up to 9 items per feed (title, link, published date, and text-only summary)
- Generates a structured JSON file with all feeds and their items

## Customizing Feeds

To change the feeds that are fetched, edit the `FEED_URLS` environment variable in `.github/workflows/update-feed.yml`:

```yaml
env:
  FEED_URLS: https://example.com/feed1.xml,https://example.com/feed2.rss
```
